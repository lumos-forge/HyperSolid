...
**设计取舍（已决策 2026-06-19）**：Trade **独立成 Tab**（用户决策），不并入 Market Detail 抽屉；Market Detail 仍可提供"快捷下单"入口跳转/预填到 Trade 板块，但交易主入口是独立 Tab。MVP 上架受合规闸门约束（§9/ADR-006）：闸门通过前公开版仅 Markets + view-only Positions，Trade/Agent 灰度或仅测试网。
