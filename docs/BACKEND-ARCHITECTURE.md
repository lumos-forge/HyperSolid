...
- **HA / 幂等**（§6.2/§12）：M4/M6 用 **Temporal(Go)** 落地持久意图账本、定时单、scheduleCancel 心跳、重试幂等；无状态多 AZ + leader 选举 + 背压 + 熔断；手动交易降级直连 HL。**【已落地（重启安全）：引擎策略状态（trailPeak/gridLimit rung/actionsDone/nextRunAt）持久化 SQLite + 确定性 cloid 重放去重 + 每日 notional 上限读持久 activity store + 死手预算 SQLite 持久化（#110）——单节点崩溃/重启可干净恢复；完整 Temporal 多 AZ + leader 选举留待 GA】**
