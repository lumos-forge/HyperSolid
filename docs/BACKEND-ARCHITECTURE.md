...
| **M4** | Agentic 执行引擎 | L1 规则（TP/SL、移动止损【落地：`kind:"trailing"`，params `{coin,trailPct}`，持久化 `trailPeak` water-mark，回撤 trailPct% 触发 reduce-only 平仓，mobile 建仓 UI 待做】、DCA、网格、条件/定时单）、护栏网关、kill-switch、scheduleCancel 心跳、策略健康（§6.x）| 否（调用 M5）| 策略误触发（受 M5 policy 边界兜底）| **Go**（Temporal）|
